...
public class PlayerConnectionListener {
    ...
    public void onConnection(PlayerConnectionEvent event, PlayerConnectionEvent.State state, Player player) {
        Log.debug("Received " + (!event.willPublish() ? "NON-PUBLISHING " : "") + state.name().toLowerCase() + " event: " + event.getPlayer().getName() + " -> " + Text.asPlain(event.getMessage()));
        if (!event.willPublish()) return;

        DiscordSRV.get().getChannelManager().getChannel("connections").ifPresent(c ->
